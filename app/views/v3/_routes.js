...
const router = govukPrototypeKit.requests.setupRouter()
        ...
        router.post('/v3/sp/did-they-attend', function(request, response) {
            if (request.session.data['did-they-attend'] == 'Yes') {
            response.redirect('/v3/sp/session-happen');
        } else if (request.session.data['did-they-attend'] == 'No') {
            response.redirect('person-attend-session-does-not-happen');
        }
        })