...
router.use(function (req, res, next) {
  ...
  res.locals.serviceHref =
    "/sprint-6/book-and-manage/manage-a-referral/manager/dashboard";
  next();
});

router.get("/dashboard", (req, res) => {
  res.locals.serviceName = "Access interventions and services";
  res.render("sprint-6/book-and-manage/manage-a-referral/manager/dashboard");
});

router.get("/referrals", (req, res) => {
  const referralsWithUnassignedInterventions = req.session.data.sprint6.referrals
    .filter((referral) =>
  ...
  const referralsWithAssignedInterventions = req.session.data.sprint6.referrals
    .filter((referral) =>
      referral.interventions.some((intervention) => intervention.assigned)
    )
    .map((referral) => {
  ...
  res.render("sprint-6/book-and-manage/manage-a-referral/manager/referrals", {
...
router.get(
  ...
  (req, res) => {
    ...
    const referral = req.session.data.sprint6.referrals[referralIndex];
    const serviceUser = referral.serviceUser;
    const intervention = findIntervention(referral, interventionId);
    const caseworkers = req.session.data.sprint6.caseworkers;
    ...
    res.render(
      "sprint-6/book-and-manage/manage-a-referral/manager/intervention",
      {
        referralIndex,
        interventionId,
        serviceUser,
...
router.get(
  ...
  (req, res) => {
    ...
    res.render(
      "sprint-6/book-and-manage/manage-a-referral/manager/confirm-details",
      {
        referralIndex,
        interventionId,
        serviceUser,
...
router.get(
  ...
  (req, res) => {
    const referral =
      req.session.data.sprint6.referrals[req.params.referralIndex];
    ...
    res.render(
      "sprint-6/book-and-manage/manage-a-referral/manager/caseworker-email-confirmation",
      {
        serviceUser,
        referralNumber,
        intervention,
...
for (const page of [
  ...
  router.get(
    ...
    (req, res) => {
      ...
      res.render(`sprint-6/book-and-manage/manage-a-referral/manager/${page}`, {
        referral,
