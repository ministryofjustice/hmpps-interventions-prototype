...
router.use(function (req, res, next) {
  ...
  res.locals.serviceHref =
    "/beta-sprint-2/book-and-manage/manage-a-referral/manager/dashboard";
  res.locals.loggedInUserName =
    req.session.data.betaSprint2.caseworkers[3].firstName;
  next();
});

router.get("/dashboard", (req, res) => {
  res.locals.serviceName = "Access interventions and services";
  res.render("beta-sprint-2/book-and-manage/manage-a-referral/manager/dashboard");
});

router.get("/referrals", (req, res) => {
  const referralsWithUnassignedInterventions = req.session.data.betaSprint2.referrals
    .filter((referral) =>
  ...
  const referralsWithAssignedInterventions = req.session.data.betaSprint2.referrals
    .filter((referral) =>
      referral.interventions.some((intervention) => intervention.assigned)
    )
    .map((referral) => {
  ...
  res.render("beta-sprint-2/book-and-manage/manage-a-referral/manager/referrals", {
...
router.get(
  ...
  (req, res) => {
    ...
    const referral = req.session.data.betaSprint2.referrals[referralIndex];
    const serviceUser = referral.serviceUser;
    const intervention = findIntervention(referral, interventionId);
    const caseworkers = req.session.data.betaSprint2.caseworkers;
    ...
    res.render(
      "beta-sprint-2/book-and-manage/manage-a-referral/manager/intervention",
      {
        referralIndex,
        interventionId,
        serviceUser,
...
router.get(
  ...
  (req, res) => {
    ...
    res.render(
      "beta-sprint-2/book-and-manage/manage-a-referral/manager/confirm-details",
      {
        referralIndex,
        interventionId,
        serviceUser,
...
router.get(
  ...
  (req, res) => {
    const referral =
      req.session.data.betaSprint2.referrals[req.params.referralIndex];
    ...
    res.render(
      "beta-sprint-2/book-and-manage/manage-a-referral/manager/caseworker-email-confirmation",
      {
        serviceUser,
        referralNumber,
        intervention,
...
for (const page of [
  ...
  router.get(
    ...
    (req, res) => {
      ...
      res.render(`beta-sprint-2/book-and-manage/manage-a-referral/manager/${page}`, {
        referral,
